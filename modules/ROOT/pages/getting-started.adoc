...
----
dependencies {
    implementation 'io.quarkiverse.couchbase:quarkus-couchbase:{quarkus_version}'
}
----
--
====
...
When using DevServices, the Bucket will automatically be created.
When connecting to an existing Couchbase instance, the bucket must already exist, and the name can be configured using `quarkus.couchbase.bucket-name`.
